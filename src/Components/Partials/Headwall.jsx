...
function Headwall({data , title}) {
  return (
    ...
    className='wall w-[100vw] h-[60vh] shadow-[0px_5px_6px_5px_#0F172A_inset] p-10 pt-20 flex flex-col justify flex-end items-start'>
        ...
        <p className='overview w-[75%] text-zinc-300  leading-tight text-m'>{data.overview.slice(0,200)} <Link to={`/${data.media_type }/details/${data.id}`} className='text-blue-500'>more</Link> </p>
