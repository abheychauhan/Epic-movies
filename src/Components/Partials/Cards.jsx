    ...
    <div className=' w-full flex gap-3'>
     {data.map((d,i)=>(
         <Link to={`/${d.media_type }/details/${d.id}`}  key={i} className="card text-sm min-w-[200px] h-[360px] p-1 bg-[#17274e] rounded-lg  flex flex-col relative   ">
             ...
             <p className='overview w-[75%] text-zinc-300  leading-tight text-m'>{d.overview.slice(0,50)} <Link to={`/${d.media_type }/details/${d.id}`} className='text-blue-500'>more</Link> </p>
